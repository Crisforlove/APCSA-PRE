...
#### 1
A client method uses an enhanced for loop to generate a list of names from an Address\[\] list. Which is the correct line of code to print each name?
...
#### 2
Which of the following is a correct way to print a list of addresses using a loop variable addr? 2

- I. System.out.println(addr.getName() + addr.getStreet()); I

- I. System.out.println(addr.toString()); I

- II. System.out.println(addr);
...
#### 3
Given a list allStudents, which code segment correctly prints each student's address name?
...
#### 4
Which strategy could correctly replace the body of a method to locate the Student with the highest idNum? 2

- I. Initializing a max variable to the first element and updating during traversal. I

- I. Sorting the list and taking the last element. I

- II. Using a nested loop to compare every ID.
...
#### 5
What happens if you attempt to add or remove elements from an ArrayList while traversing it using an **enhanced for loop**?
...
#### 6
When **deleting elements** during a traversal of an ArrayList, which technique is necessary to avoid skipping elements?
...
#### 7
Which method is used to access all or an ordered sequence of elements in an ArrayList?
...
#### 8
Accessing an index value outside of the range 0 to size \- 1 in an ArrayList results in:
...
#### 9
Which statement about a for-each loop variable in an ArrayList traversal is true?
...
#### 10
To safely remove elements while traversing, which Java object provides a remove() method that does not skip elements?
...
#### 1
A "compression" algorithm for an ArrayList moves only non-zero elements into a new list. If the original list is \[6-8\], what is the result?
...
#### 2
Which of the following is a standard ArrayList algorithm to determine the presence of duplicate elements?
...
#### 3
To find the **minimum** or **maximum** value in an ArrayList, you should:
...
#### 4
Which method is used to insert an element at a specific position index in an ArrayList, shifting subsequent elements to the right?
...
#### 5
A standard algorithm to **reverse** the order of elements in an ArrayList can be implemented by:
...
#### 6
What is the effect of calling list.remove(index)?
...
#### 8
Some algorithms require multiple ArrayList objects to be traversed **simultaneously**. An example is:
...
#### 9
To compute the **average** of numerical values in an ArrayList, the algorithm must:
...
#### 10
The standard algorithm to **shift or rotate** elements left or right typically requires:
...
#### 1
Consider two nested while loops. Loop A prints $m \\times n$ times. Loop B prints $m \\times (n-1)$ times. Which is true?
...
#### 4
When accessing arr\[first\]\[second\], what do the indices represent?
...
#### 5
For a 2D array named values, how do you find the number of **rows**?
...
#### 6
For a 2D array named values, how do you find the number of **columns** (assuming it is rectangular)?
...
#### 7
What are the default values for elements in a 2D int array created with new?
...
#### 8
A single row of a 2D array can be accessed using:
...
#### 9
What exception is thrown if you access arr\[arr.length\]?
...
#### 1
**Row-major order** traversal occurs when:
...
#### 2
**Column-major order** traversal occurs when:
...
#### 3
In a nested enhanced for loop traversing a 2D array int\[\]\[\] arr, what is the type of the **outer** loop variable?
...
#### 4
In a nested enhanced for loop traversing a 2D array int\[\]\[\] arr, what is the type of the **inner** loop variable?
...
#### 5
What is the result of assigning a new value to the **enhanced for loop variable** during a 2D array traversal?
...
#### 6
How many times does the inner loop execute in a row-major traversal of an $N \\times M$ array?
...
#### 7
Which loop structure is best for a column-major traversal of int\[\]\[\] mat?
...
#### 1
Which algorithm involves finding the sum of all elements in a **designated row**?
...
#### 2
To determine if **duplicate elements** exist in a subsection of a 2D array, which approach is required?
...
#### 3
What does the following code segment do? (Assuming a square matrix mat and a swap method).
...
#### 4
The method alter is intended to **remove column c**. Which change will correct a buggy implementation? 24

- I. Change the loop limit to mat.length \- 1\. I

- I. Shift elements to the left by assigning mat\[i\]\[j\] \= mat\[i\]\[j+1\]. I

- II. Both of the above are valid steps in the correction.
...
#### 5
A method isThere(mat, r, c, target) returns true if a specific condition is met. Which condition guarantees a true result if the logic checks rows/columns?
...
#### 6
Standard 2D algorithms can determine a minimum or maximum value:
...
#### 7
Which algorithm correctly **reverses the order** of elements in a row?
...
#### 8
To **shift or rotate** elements in a column up or down, the algorithm must:
...
#### 9
Determining if **all elements** in a designated subsection have a particular property requires:
...
#### 10
Calculating the **average** of a designated column involves:
...
#### 1
The decision to choose a particular searching or sorting algorithm should be based on: 25

- I. Run-time efficiency. I

- I. Size of the array. I
...
#### 2
Which boolean expression should replace the placeholder in a **sequential search**?
...
#### 3
What feature is required for a **binary search** but not necessarily for a sequential search?
...
#### 4
For which operation is a **sorted array** more efficient than an unsorted one? 25

- I. Inserting a new element. I

- I. Searching for a given element. I
...
#### 5
In what case does a "jump" search algorithm (comparing every third item) use **fewer** comparisons than a binary search?
...
#### 6
If the assertion a \> a\[k\] for all $0 \< k \< N$ is true, what must be true?
...
#### 7
```java
A sequential search implementation while(a[index] != key) index++; will definitely fail if:
```
...
#### 8
If a search method contains while (v\[index\] \< key), what should be added to the **precondition**?
...
#### 10
When applying a linear search to a **2D array**:
...
#### 1
Which sorting algorithm repeatedly selects the smallest element from the unsorted portion and **swaps** it into its correct position?
...
#### 2
Which sorting algorithm inserts an element into its correct position in the sorted portion by **shifting** other elements?
...
#### 3
What would the array {89, 42, -3, 13, 109, 70, 2} look like after the **third pass** of a selection sort (sorting high to low)?
...
#### 4
**Selection sort** and **insertion sort** are categorized as:
...
#### 6
Which algorithm is generally more efficient for sorting **long, randomly ordered** lists in the AP CSA curriculum?
...
#### 8
Selection sort is preferred over insertion sort when:
...
#### 9
In **Selection Sort**, the number of **comparisons** is always:
...
#### 10
The "sorted portion" of the list in **Insertion Sort** grows by:
...
#### 1
Which of the following statements about **recursion** are true? 32

- I. Every recursive algorithm can be written iteratively. I

- I. Tail recursion is always used in "divide-and-conquer". I

- II. In a recursive definition, a process is defined in terms of a simpler case of itself.
...
#### 3
What value does the recursive call result(5) return if it calculates $2^n$?
...
#### 4
How many total calls (including the initial) are made to evaluate result(n) for $n \> 0$ if each call decreases $n$ by 1?
...
#### 5
If a recursive method stringRecur has **no base case**, what will happen?
...
#### 6
A recursive method that prints a character **after** the recursive call will:
...
#### 7
What is stored in each recursive call to keep track of progress?
...
#### 9
A recursive method for $n\!$ should return:
...
#### 10
Is writing recursive code required on the AP Computer Science A exam?
...
#### 1
Which recursive sorting algorithm divides an array into smaller subarrays until each has one element and then merges them back in order?
...
#### 2
**Binary Search** is typically more efficient than Linear Search because:
...
#### 3
A recursive method doSomething(3) that calls itself twice per level results in which output sequence?
...
#### 4
To use the **Binary Search** algorithm, the data MUST be:
...
#### 5
What is the result of each step of the **Merge Sort** algorithm?
...
#### 6
Which change in the recursive method writeWithCommas will fix an implementation that prints incorrectly?
...
#### 8
What is the return value of foo(foo(3) \+ foo(4)) if foo(n) calculates $n\!$?
...
#### 9
Recursion can be used to traverse which data collections?
...
#### 10
In a recursive binary search, if the target is **not found**:
