...
#### 2
An attendance application only requires a student's name and ID number to function. Which approach is most consistent with **data minimization**?
...
#### 4
What is the best immediate mitigation after detecting unauthorized access to a database containing student records?
...
#### 5
A fitness tracking app shares raw heart-rate data with third-party advertisers. What is the primary **ethical issue**?
...
#### 6
A company's policy states that server logs are deleted after 30 days, but a developer keeps them for a year. What is the main risk?
...
#### 8
A survey application only collects responses from "Premium" users. What type of **bias** does this risk?
...
#### 9
Which measure is most appropriate for protecting **sensitive data at rest** (e.g., student grades on a disk)?
...
#### 10
If a programmer collects email addresses for "notifications" but later wants to sell them to advertisers, they should:
...
#### 1
Why use an **ArrayList** of Strings rather than an **Array** to read lines from a file of unknown length?
...
#### 2
Which is the best reason to use an **array of int** rather than an **ArrayList of Integer** for long numerical lists?
...
#### 4
In a class constructor, which code correctly initializes an array tickList of Ticket objects?
...
#### 5
Which code segment represents a correct implementation for calculating a sum in a totalPaid method?
...
#### 6
A programmer is writing a loop to find the minimum value. How can they avoid skipping the last element?
...
#### 7
Given ArrayList strList, which of the following will cause a compiler error?
...
#### 3
What are the initial values of elements in a new int\[69\] array?
...
#### 6
Consider: int[] a = {1, 2, 3}; for (int x : a) { x = x * 2; }. What is a after execution?
...
#### 7
Consider: int[] a = {1, 2, 3}; int[] b = a; b = 9;. What is a?
...
#### 8
Which is a correct strategy to locate the Student with the **highest ID**? 48

- I. Maintain a max variable during traversal. I

- I. Sort and return the first element. I

- II. Sort and return the last element.
...
#### 1
Which method correctly initializes arr to contain four zeros? 48

- I. int[] arr = {0, 0, 0, 0}; I

- I. int[] arr = new int[67]; I

- II. Using a for loop to assign 0.
...
#### 7
Which is a correct strategy for finding the **highest ID**? 49

- I. Init max to first element. I

- I. Compare each subsequent element to max. I

- II. Update max when higher value is found.
...
#### 1
Why is an **ArrayList** preferred over an **Array** for reading sequential file lines?
...
#### 2
When is a missing division-by-zero test detected while reading file expressions?
...
#### 3
What is the most reliable way to count file lines with Scanner?
...
#### 4
Result of failing to call close() on a Scanner?
...
#### 6
new File("data/input.txt") reports "File Not Found". Most likely cause?
...
#### 7
Which class is most appropriate for **writing** text line-by-line?
...
#### 8
Exception thrown by new Scanner(new File("missing.txt")) if file doesn't exist?
...
#### 9
Most efficient Scanner method to count **individual word** frequency?
...
#### 1
Consider d1 \+ d2 \+ d3.doubleValue() \+ d4. What is the type and value if total is 100.0?
...
#### 2
Passing Integer val \= 10 to a method accepting int results in:
...
#### 3
Given Double y = 3 / 2.0; y /= 2;, what is y.doubleValue()?
...
#### 4
Which expression results in random integer 1 to 10 inclusive?
...
#### 5
Assigning Double to double primitive variable:
...
#### 6
Best practice to compare two Integer objects a and b both at 128?
...
#### 7
Which Integer method is preferred over the constructor for performance?
...
#### 8
Difference between parseInt("42") and valueOf("42")?
...
#### 9
Correctly round negative double d to nearest integer?
...
#### 10
Why avoid \== for floating-point results like 0.1 \+ 0.2 and 0.3?
...
#### 1
Advantage of **ArrayList** over **Array** when reading a file?
...
#### 2
Implementation more efficient for high-performance math?
...
#### 3
Correct line to print name using for (Address a : list)?
...
#### 4
Valid ways to print Address addr (toString overridden)? 57

- I. addr.name \+ addr.street I

- I. System.out.println(addr) I

- II. System.out.println(addr.toString())
...
#### 5
Given ArrayList allStudents, which prints student's address name?
...
#### 6
Strategy for finding **highest ID** in an ArrayList?
...
#### 7
True about accessing/modifying an ArrayList?
...
#### 8
How to retrieve number of elements?
...
#### 9
Return values of add(E e) and remove(int index)?
